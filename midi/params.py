...
_PARAMS: list[ParamDef] = [
    ...
    *[ParamDef(f"vocoder_pan_{i+1}", f"Vocoder band {i+1} pan",
    ...
    # -----------------------------------------------------------------------
    # Virtual Patch Intensity (NRPN destination values, SysEx-only)
    # These have SysEx offsets but no NRPN address of their own.
    # Intensity is set via virtual patch destination value mapping.
    # -----------------------------------------------------------------------
    *[ParamDef(f"patch{i}_intensity", f"Virtual Patch {i} intensity",
    ...
    # -----------------------------------------------------------------------
    # Timbre 1: OSCILLATOR 1  (SysEx-only — offsets TBD via sysex_diff)
    # -----------------------------------------------------------------------
    ...
    *[ParamDef(f"arp_step{i}_sw", f"Arp Step {i} on/off", f"Step {i} on/off toggle",
    ...
    # -----------------------------------------------------------------------
    ParamDef("long_ribbon_scale_sw", "Long Ribbon Scale SW", "Scale function on/off",
             0, 1, group="ribbon", section="long_ribbon",
             display_name="Scale SW",
             value_labels={0: "Off", 1: "On"}),
    ...
    ParamDef("long_ribbon_scale_type", "Long Ribbon Scale Type", "Scale type (35 types)",
